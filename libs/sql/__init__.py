...
def is_local_domain(conn,
    ...
    try:
        ...
        logger.debug(f"[SQL] query local domain ({domain}): \n{sql}")

        qr = conn.execute(sql)
        sql_record = qr.fetchone()
        logger.debug(f"SQL query result: {repr(sql_record)}")

        if sql_record:
            return True
    except Exception as e:
        logger.error(f"<!> Error while querying domain: {repr(e)}")
    ...
    try:
        if include_alias_domain:
            ...
            logger.debug(f"[SQL] query alias domain ({domain}): \n{repr(sql)}")
...
def get_alias_target_domain(alias_domain, conn):
    ...
    if not utils.is_domain(alias_domain):
        logger.debug(f"Given alias domain ({alias_domain}) is not a valid domain name.")
        return None
    ...
    logger.debug(f"[SQL] query target domain of given alias domain ({alias_domain}): \n{repr(sql)}")
...
def get_access_policy(mail, account_type, conn):
    ...
    logger.debug(f"[SQL] query access policy: \n{sql}")

    qr = conn.execute(sql)
    record = qr.fetchone()
    logger.debug(f"[SQL] query result: {repr(record)}")
